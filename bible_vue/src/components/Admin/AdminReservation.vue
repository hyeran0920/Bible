<template>
  <div class="reservation-container">
    ...
    <!-- 책 정보 모달 -->
    <div class="modal" v-if="showBookModal" @click="closeBookModal">
        <div class="modal-content" @click.stop>
            <h3>도서 정보</h3>
            <div v-if="bookInfo" class="book-info">
                <router-link :to="'/book/' + bookInfo.bookId">
                    <img :src="getBookImageUrl(bookInfo.bookId)" 
                ...
                <table class="book-details-table">
                    ...
                    <tr>
                        <th>카테고리</th>
                        <td>{{ bookInfo.bookCategory }}</td>
                    </tr>
                    <tr>
                        <th>가격</th>
                        <td>{{ bookInfo.bookPrice }}원</td>
                    </tr>
                    <tr>
                        <th>출간일</th>
                        <td>{{ bookInfo.bookReleaseDate }}</td>
                    </tr>
                    <tr>
                        <th>총재고</th>
                        <td>{{ bookInfo.bookTotalStock }}</td>
                    </tr>
                    <tr>
                        <th>대여 가능 재고</th>
                        <td>{{ bookInfo.bookRentStock }}</td>
                    </tr>
                </table>
            </div>
            <button class="close-btn" @click="closeBookModal">닫기</button>
        </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async showBookInfo(bookId) {
        ...
        try {
            const imageResponse = await this.$axios.get(`/uploads/book-image?bookid=${bookId}`);
            this.bookImageUrl = imageResponse.data;
        } catch (error) {
            this.bookImageUrl = null; // 이미지가 없는 경우
        }
